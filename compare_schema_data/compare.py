...
def get_prev_version(db: SchemaClient, version: str) -> str:
    """get previous version"""

    row = db.read_row("read_prev_version", {"version": version})
    return row["version"] if row else ""
    ...
    *,
    ...
    if not sqlite_path.exists():
        raise FileNotFoundError(
            f"sqlite_file is not found: {sqlite_path}, run import is required"
        )
    ...
    with SchemaClient(database=sqlite_path) as db:
        ...
        is_same_data, compare_types_data = get_same_data_by_version(
            db, version, prev_version, config.data_tables
        )
        if is_same_schema:
            print(f"same schema between {version} and {prev_version}")
        else:
            print(f"compare_types_schema: {compare_types_schema}")

        if is_same_data:
            print(f"same data between {version} and {prev_version}")
        else:
            print(f"compare_types_data: {compare_types_data}")

        schema_not_exists_list, schema_diff_list = compare_schema(
            db, compare_types_schema, version, prev_version, config
        )
        ...
        if config.output_type == "xlsx":
            xlsx_path = output_dir / f"{version}_compare_log.xlsx"
        ...
        elif config.output_type == "md":
            excluded_tables = (
                ", ".join(config.exclude_tables) if config.exclude_tables else "None"
            )
            excluded_columns = (
                ", ".join(config.exclude_columns) if config.exclude_columns else "None"
            )
            remark = (
                f"- **version**: {version}\n"
                f"- **excluded tables**: {excluded_tables}\n"
            ...
            write_markdown(
                ...
                data_diff_list=data_diff_list,
                remark=remark,
                md_path_compare=md_path_compare,
